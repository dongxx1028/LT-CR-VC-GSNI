...
% When L=1.2,2.1,3,3.9,4.8, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for the characteristic roots of the characteristic equation of the right wing of the fracture: equations（48）-（49）
       lambda1r=(vf1r+sqrt(vf1r^2+4*D*(s-zetar)))/(2*D);
       lambda2r=(vf1r-sqrt(vf1r^2+4*D*(s-zetar)))/(2*D);
...
% Code for the concentration of contaminanton at the bottom of the wellboree: equation（90） 
       C1b=exp(lambda21b*L(k))*((Cf1L+ Cf1r)/2+(Ab+Bb*lambda11b)/((lambda21b-lambda11b)*lambda11b^2)+(Ab+Bb*lambda21b)/((lambda11b-lambda21b)*lambda21b^2))...
           -(Ab*lambda11b*L(k)+Ab+Bb*lambda11b)/((lambda21b-lambda11b)*lambda11b^2)-(Ab*lambda21b*L(k)+Ab+Bb*lambda21b)/((lambda11b-lambda21b)*lambda21b^2);
% Code for the concentration of contaminanton at the wellhead: equation（91）   
       Cbe=exp(lambda2be*H)*(C1b+(Ae+Be*lambda1be)/((lambda2be-lambda1be)*lambda1be^2)+(Ae+Be*lambda2be)/((lambda1be-lambda2be)*lambda2be^2))...
           -(Ae*lambda1be*H+Ae+Be*lambda1be)/((lambda2be-lambda1be)*lambda1be^2)-(Ae*lambda2be*L(k)+Ae+Be*lambda2be)/((lambda1be-lambda2be)*lambda2be^2);
...
legend('$$ L=1.2 $$','$$ L=2.1 $$','$$ L=3 $$','$$ L=3.9 $$','$$ L=4.8 $$','Interpreter','latex')
box off 
grid on  
