...
% When H=1.2,2.1,3,3.9,4.8, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for the characteristic roots of the characteristic equation from the bottom of the vertical wellbore to the wellheade: equations£¨74£©-£¨74£© 
...
% Code for the concentration of contaminanton at the wellhead: equation£¨91£©   
       Cbe=exp(lambda2be*H(k))*(C1b+(Ae+Be*lambda1be)/((lambda2be-lambda1be)*lambda1be^2)+(Ae+Be*lambda2be)/((lambda1be-lambda2be)*lambda2be^2))...
           -(Ae*lambda1be*H(k)+Ae+Be*lambda1be)/((lambda2be-lambda1be)*lambda1be^2)-(Ae*lambda2be*L+Ae+Be*lambda2be)/((lambda1be-lambda2be)*lambda2be^2);
...
legend('$$ H=1.2 $$','$$ H=2.1 $$','$$ H=3 $$','$$ H=3.9 $$','$$ H=4.8$$','Interpreter','latex')
box off 
grid on   