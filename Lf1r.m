...
% When Lf1r=0.08,0.14,0.2,0.26,0.32, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for equations£¨80£©-£¨81£©
       ...
       d2=a2*(1-1/(2*n))*Lf1r(k)+e2-c2*h2^2;
...
% Code for the concentration of contaminanton the right wing of the first fracture: equation£¨87£©
        Cf1r=(exp(lambda2r*(1/(2*n)-1)*Lf1r(k))*(q2+(lambda1r*a2*(1-1/(2*n))*Lf1r(k)+a2+e2*lambda1r)/...
           ((lambda2r-lambda1r)*lambda1r^2)+(lambda2r*a2*(1-1/(2*n))*Lf1r(k)+a2+e2*lambda2r)/((lambda1r-lambda2r)*lambda2r^2))...
...
legend('$$L_{f1r}=0.08 $$','$$ L_{f1r}=0.14 $$','$$ L_{f1r}=0.2$$','$$ L_{f1r}=0.26$$','$$ L_{f1r}=0.32 $$','Interpreter','latex')
box off
grid on   
