...
% When v1b=2.48,4.34,6.2,8.06,9.92, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for the characteristic roots of the characteristic equation from the first crack to the bottom of the vertical wellbore: equations£¨68£©-£¨69£© 
       lambda11b=(v1b(k)+sqrt(v1b(k)^2+4*D*s))/(2*D);
       lambda21b=(v1b(k)-sqrt(v1b(k)^2+4*D*s))/(2*D);
% Code for the characteristic roots of the characteristic equation from the bottom of the vertical wellbore to the wellheade: equations£¨83£©-£¨84£© 
...
legend('$$v_{1b}=2.48 $$','$$v_{1b}=4.34$$','$$v_{1b}=6.2$$','$$v_{1b}=8.06$$','$$v_{1b}=9.92$$','Interpreter','latex')
box off 
grid on   