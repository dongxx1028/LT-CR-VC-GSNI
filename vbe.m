...
% When vbe=2.4,4.2,6,7.8,9.6, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for the characteristic roots of the characteristic equation from the bottom of the vertical wellbore to the wellheade: equations£¨73£©-£¨74£© 
       lambda1be=(vbe(k)+sqrt(vbe(k)^2+4*D*s))/(2*D);
       lambda2be=(vbe(k)-sqrt(vbe(k)^2+4*D*s))/(2*D);
...
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.5)
hold on 
...
legend('$$ v_{be}=2.4 $$','$$ v_{be}=4.2 $$','$$ v_{be}=6 $$','$$ v_{be}=7.8 $$','$$ v_{be}=9.6 $$','Interpreter','latex')
box off  
grid on  
