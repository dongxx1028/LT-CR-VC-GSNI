...
% When vfl=2.2,3.85,5.5,7.15,8.8, reverse solution of Laplace space  concentration of contaminants at the wellhead (equation (91)) to real space concentration of contaminants
...
% Code for the characteristic roots of the characteristic equation of the left wing of the fracture.: equations£¨34£©-£¨35£©
       lambda1L=(vf1l(k)+sqrt(vf1l(k)^2+4*D*(s-zetaL)))/(2*D);
       lambda2L=(vf1l(k)-sqrt(vf1l(k)^2+4*D*(s-zetaL)))/(2*D);
...
legend('$$ \overline{v}_{f1l}=2.2 $$','$$ \overline{v}_{f1l}=3.85 $$','$$ \overline{v}_{f1l}=5.5 $$','$$ \overline{v}_{f1l}=7.15 $$','$$ \overline{v}_{f1l}=8.8$$','Interpreter','latex')
box off 
grid on   